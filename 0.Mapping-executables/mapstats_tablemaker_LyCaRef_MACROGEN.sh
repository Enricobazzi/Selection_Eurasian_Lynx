...
BamDir_PATH=/mnt/lustre/scratch/home/csic/ebd/jg2/LL_selection/LyCaRef_bams
sample=($(echo ${1}))
SIZEGENOME=($(cat ${LUSTRE}/LL_selection/Ref_Genome_LyCa/lc4.fa.fai | awk '{sum+=$2} END {print sum}'))
BamStats="${sample}"_LyCa_ref_sorted_rg_rmdup_sorted_indelrealigner.stats
BAM="${sample}"_LyCa_ref_sorted_rg_rmdup_sorted_indelrealigner.bam
INfastq_PATH=($(echo ${2}))
# This is only good for CNAG format of Lynx lynx fastqs
SenderID=($(for key in $(echo ${!BARCODEID[@]}); do echo -e "${key}\t${BARCODEID[${key}]}"; done | grep ${sample} | cut -f1 | sort -u))
...
TOTAL_SEQ=($(zcat $INfastq_PATH/*"${SenderID}"*_trimmed.fastq.gz | wc -l | awk '{print $1/4}'))
...
DUPLICATES=($(grep "duplicates" $BamDir_PATH/"${BamStats}" | cut -d "+" -f 1))

# column 5 : mapped --> mapped reads
MAPPED=($(grep "0 mapped" $BamDir_PATH/"${BamStats}" | cut -d "+" -f 1))

# column 6 : properly_pair --> pairs of reads properly paired
PROPERLY_PAIR=($(grep "properly paired" $BamDir_PATH/"${BamStats}" | cut -d "+" -f 1))

# column 7 : with_mat_to_another_chr --> reads with mate mapped to another chromosome
WITH_MATE_TO_ANOTHER_CHR=($(grep "with mate mapped to a different chr$" $BamDir_PATH/"${BamStats}" | cut -d "+" -f 1))
...
# column 14 : samtools_depth --> mean depth and standard deviation calculated with samtools depth
SAMTOOLS_DEPTH_mean_stdev=($(samtools depth ${BamDir_PATH}/${BAM} | awk -v OFS=',' -v sizegenome=${SIZEGENOME} '{sum+=$3; sumsq+=$3*$3} END { print sum/sizegenome , sqrt(sumsq/sizegenome - (sum/sizegenome)**2)}'))

echo "$NAME,$TOTAL_SEQ,$TOTAL_READS,$DUPLICATES,$MAPPED,$PROPERLY_PAIR,$WITH_MATE_TO_ANOTHER_CHR,$UNIQ_MAPPED,$UNIQ_MAPPED_vs_TOTAL_READS,$DUPLICATES_vs_TOTAL_READS,$MAPPED_vs_TOTAL_SEQ,$PROPERLY_PAIR_vs_TOTAL_SEQ,$WITH_MATE_TO_ANOTHER_CHR_vs_TOTAL_SEQ,$SAMTOOLS_DEPTH_mean_stdev" > $BamDir_PATH/"${sample}".mapstats.csv
